...
class PostRepository{
  ...
  Future<void> curtirPost(int idPost, String idUser) async{
    ...
    if(response.isNotEmpty) {
      await supabase.from('curtidas').delete().eq('id_post', idPost).eq('id_user', idUser);
      return;
    }
    try {
      await supabase.from('curtidas').insert({'id_post': idPost, 'id_user': idUser});
    } catch (e) {
      print(e);
    }
  ...
  Future<int> getQuantidadeCurtidas(int idPost) async{
    try {
      var response = await supabase.from('curtidas').select('*', const FetchOptions(count: CountOption.exact)).eq('id_post', idPost);
  ...
  Future<bool> isCurtido(int idPost, String idUser) async{
    try {
      var response = await supabase.from('curtidas').select('id_post').eq('id_post', idPost).eq('id_user', idUser);
